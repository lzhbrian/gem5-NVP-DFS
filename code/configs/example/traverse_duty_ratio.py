...
# Last edit at: 2017.7.2 11:44
# usage: python traverse_duty_ratio.py
...
fp = open('traverse_duty_result_time_count.txt','w')

for duty_ratio in traverse_list: # np.linspace(0, 10, 21):
    if high == 0:
        continue
    engy_prof_filename = os.path.join(foldername, str(duty_ratio)) + '.txt'
    result_filename = 'traverse_duty_result.txt'
    print 'to exec:', basics + ' ' + generate_param(engy_prof_filename, 1000000, 500000, result_filename)

    status, output = commands.getstatusoutput(basics + ' ' + generate_param(engy_prof_filename, 1000000, 500000, result_filename))
    on_off_count = 0
    freq_switch_count = 0
